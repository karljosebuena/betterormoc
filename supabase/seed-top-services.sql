-- Seed data for top priority services: Marriage Certificate, Death Certificate, CENOMAR, Barangay Clearance
-- All data verified from official Philippine government sources (2025)
...
SELECT 
  ...
  'marriage-certificate',
  'Certificates',
  'Request a certified true copy of your marriage certificate from the Local Civil Registrar Office. Required for passport applications, visa processing, legal transactions, and other official purposes.',
  '₱150 per copy',
  '15-30 minutes',
  ...
  '{"owner": ["Valid government-issued ID of either spouse (original + photocopy)", "Payment: ₱150 cash per copy"], "representative": ["Authorization letter signed by either spouse", "Valid ID of the spouse (photocopy)", "Valid ID of the representative (original + photocopy)", "Payment: ₱150 cash per copy"]}'::jsonb,
  ARRAY['birth-certificate', 'death-certificate', 'cenomar']
...
SELECT 
  ...
  'death-certificate',
  'Certificates',
  'Request a certified true copy of a death certificate from the Local Civil Registrar Office. Required for estate settlement, insurance claims, pension benefits, and other legal purposes.',
  '₱150 per copy',
  '15-30 minutes',
  'Local Civil Registrar',
  '["Valid government-issued ID", "Payment fee"]'::jsonb,
  ...
  ARRAY['birth-certificate', 'marriage-certificate']
...
SELECT 
  ...
  'cenomar',
  'Certificates',
  'Certificate of No Marriage Record from the Philippine Statistics Authority (PSA). Required for marriage license applications, visa processing, employment abroad, and other legal purposes. This certifies that you have not contracted any marriage in the Philippines.',
  '₱210 (walk-in) / ₱420 (online with delivery)',
  '1-3 business days (walk-in) / 2-7 days (online)',
  'Philippine Statistics Authority (PSA)',
  '["Valid government-issued ID", "Complete personal information", "Payment fee"]'::jsonb,
  '[
  ...
  '{"name": "Philippine Statistics Authority (PSA)", "location": "Various PSA CRS Outlets nationwide", "address": "Check PSA website for nearest outlet", "phone": "1388 (PSA Hotline)", "hours": "Monday to Friday: 8:00 AM - 5:00 PM (varies by outlet)", "map_coordinates": null}'::jsonb,
...
SELECT 
  ...
  'barangay-clearance',
  'Clearances',
  'Certificate issued by the Barangay confirming that you are a resident and have no derogatory records in the barangay. Required for employment, business permits, police clearance, and other transactions.',
  '₱50-₱100 (varies by barangay)',
  '30 minutes - 1 day',
  'Barangay Hall',
  '["Valid ID", "Proof of residency", "Cedula", "Payment fee"]'::jsonb,
  '[
  ...
  '{"name": "Barangay Hall", "location": "Your Barangay of Residence", "address": "Ormoc City has 110 barangays - visit your local barangay hall", "phone": "Contact your barangay office", "hours": "Monday to Friday: 8:00 AM - 5:00 PM (varies by barangay)", "map_coordinates": null}'::jsonb,
  ...
  ARRAY['community-tax-certificate', 'police-clearance', 'business-permit']
WHERE NOT EXISTS (SELECT 1 FROM services WHERE slug = 'barangay-clearance');

-- Add Community Tax Certificate (Cedula) as a related service
INSERT INTO services (title, slug, category, description, fees, processing_time, office)
SELECT 
  'Community Tax Certificate (Cedula)',
  'community-tax-certificate',
  'Clearances',
  'Also known as Cedula, this is a tax certificate required for various transactions including barangay clearance, business permits, and other government services.',
  '₱5-₱50 (based on income)',
  '15-30 minutes',
  'Barangay Hall or Municipal Treasurer Office'
WHERE NOT EXISTS (SELECT 1 FROM services WHERE slug = 'community-tax-certificate');

-- Add Police Clearance as a related service
INSERT INTO services (title, slug, category, description, fees, processing_time, office)
SELECT 
  'Police Clearance',
  'police-clearance',
  'Clearances',
  'National Bureau of Investigation (NBI) or Philippine National Police (PNP) clearance certificate.',
  '₱115-₱170',
  '3-5 business days',
  'NBI or PNP Office'
WHERE NOT EXISTS (SELECT 1 FROM services WHERE slug = 'police-clearance');
