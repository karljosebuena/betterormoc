...
export function StatisticsCharts({ statistics }: StatisticsChartsProps) {
  ...
  const historicalData = statistics
    .filter(
      (s) =>
        s.category === 'Historical Trends' && s.label.includes('Population')
    )
  ...
  const agricultureData = statistics
    .filter(
      (s) =>
        s.category === 'Agriculture Details' && s.label.includes('Production')
    )
    .map((s) => ({
  ...
  const educationData = statistics
    .filter(
      (s) => s.category === 'Education Details' && s.label.includes('Students')
    )
    .map((s) => ({
  ...
  const healthData = [
    {
      label: 'Hospitals',
      value: parseInt(
        statistics.find((s) => s.label === 'Hospitals')?.value || '0'
      ),
    },
    {
      label: 'Rural Health Units',
      value: parseInt(
        statistics.find((s) => s.label === 'Rural Health Units')?.value || '0'
      ),
    },
    {
  ...
  const officialBusinessCount =
    statistics.find((s) => s.label === 'Number of Business Establishments')
      ?.value || '10,736'
    ...
    <div className="space-y-12">
        ...
        <div className="rounded-lg border border-gray-200 bg-white p-8 shadow-sm">
          <div className="mb-6">
            ...
            <p className="mt-1 text-gray-600">
              Historical population data from 1990 to 2024
            </p>
          </div>
      ...
      {hasBarangayData && (
        <div className="rounded-lg border border-gray-200 bg-white p-8 shadow-sm">
          <div className="mb-6">
            <h2 className="text-2xl font-bold text-gray-900">
              Top 20 Barangays by Population
            </h2>
            <p className="mt-1 text-gray-600">
              Most populous barangays in Ormoc City
            </p>
          </div>
