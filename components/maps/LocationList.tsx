                ...
                {Object.entries(groupedLocations).map(([type, locs]) => (
                    <div key={type}>
                        <h3 className="mb-3 text-sm font-semibold uppercase tracking-wide text-gray-500">
                            {locationTypeLabels[type as keyof typeof locationTypeLabels]} ({locs.length})
                        </h3>
                        ...
                        <div className="space-y-2">
                            {locs.map((location) => (
                                <button
                                    ...
                                    className={`w-full rounded-lg border p-4 text-left transition-all ${selectedLocation === location.id
                                ...
                                >
                                    <div className="mb-2 flex items-start justify-between">
                                        ...
                                        <MapPin
                                            className={`h-5 w-5 flex-shrink-0 ${selectedLocation === location.id ? 'text-blue-900' : 'text-gray-400'
                                                }`}
                                        />
                                    </div>
