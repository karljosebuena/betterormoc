  ...
  const filteredProjects =
    filter === 'all'
      ...
      : DPWH_PROJECTS.filter((p) => {
          ...
          if (filter === 'buildings')
            return p.category === 'Building' || p.category === 'Bridge'
          return true
        })
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        >
          Flood Control (
          {DPWH_PROJECTS.filter((p) => p.category === 'Flood Control').length})
        </button>
        <button
        ...
        >
          Buildings (
          {
            DPWH_PROJECTS.filter(
              (p) => p.category === 'Building' || p.category === 'Bridge'
            ).length
          }
          )
        </button>
            ...
            <div className="mb-4 flex flex-wrap items-start justify-between gap-2">
              <div className="flex-1">
                <div className="mb-2 flex flex-wrap items-center gap-2">
                  ...
                  <span
                    className={`rounded-full px-2.5 py-0.5 text-xs font-medium ${getCategoryBadgeColor(project.category)}`}
                  >
                    {project.category}
                  </span>
                  {project.status === 100 ? (
      ...
      <div className="rounded-lg border border-blue-100 bg-blue-50 p-4">
        <p className="text-sm text-blue-900">
          <strong>Data Source:</strong> DPWH Transparency Portal and DPWH Leyte
          2nd District Engineering Office. Sample data shown for demonstration.
          Contact DPWH for official project updates.
        </p>
      </div>
    </div>
  )
